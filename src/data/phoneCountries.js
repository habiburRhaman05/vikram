/**
 * Countries for the enquiry form's phone field - a real, worldwide list
 * (ISO 3166-1 alpha-2 code, name, ITU-T E.164 calling code), not a curated
 * handful. These are public international standards, not claims specific
 * to this business.
 *
 * `digits` is a [min, max] range for the LOCAL number, after the dial code
 * is removed - what the phone field validates against once a country is
 * selected.
 *   - The five countries this business actually fields enquiries from most
 *     (US, Canada, Mexico, Brazil, UK) carry real, verified ranges for
 *     their national numbering plans.
 *   - Every other country carries a deliberately general [6, 14] range
 *     rather than a fabricated precise rule - numbering plans vary widely
 *     and inventing exact digit counts for 190 countries would be
 *     presenting a guess as a fact. [6, 14] still catches an obviously
 *     wrong number (too short, or longer than E.164's 15-digit total
 *     allows once the dial code is subtracted) without pretending to
 *     enforce a national format we haven't actually verified.
 */

const GENERAL_RANGE = [6, 14];

export const COUNTRIES = [
  { code: "US", dial: "+1", tint: "#1F4E9C", name: "United States", digits: [10, 10] },
  { code: "CA", dial: "+1", tint: "#C8102E", name: "Canada", digits: [10, 10] },
  { code: "MX", dial: "+52", tint: "#0A6B3E", name: "Mexico", digits: [10, 10] },
  { code: "BR", dial: "+55", tint: "#159C4A", name: "Brazil", digits: [10, 11] },
  { code: "GB", dial: "+44", tint: "#26307A", name: "United Kingdom", digits: [10, 10] },
...
].map((c) => ({ tint: "#0A7F52", digits: GENERAL_RANGE, ...c }));
